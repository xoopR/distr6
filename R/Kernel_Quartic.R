      ...
      for (i in seq_along(x)) {
        if (abs(x[i]) >= 2) {
          ...
          if (upper[i] <= x[i] - 1) {
            ...
            ret[i] <- (15 / 16)^2 * (1 / 630) *
              (- x[i]^9 + 24 * x[i]^7 - 336 * x[i]^5 + 672 * x[i]^4 - 768 * x[i]^2 + 512)
          }
        } else if (x[i] <= 0) {
          if (upper[i] <= -1) {
            ret[i] <- 0
            ...
            ret[i] <- (15 / 16)^2 * (upper[i] * (x[i]^4 - 2 * x[i]^2 + 1) + (8 * x[i]^4) / 15 +
                                        ...
                                        upper[i]^6 * (2 * x[i] - (2 * x[i]^3) / 3) +
                                        upper[i]^2 * (2 * x[i] - 2 * x[i]^3) - (64 * x[i]^2) / 105 +
                                        upper[i]^7 * ((6 * x[i]^2) / 7 - 4 / 7) -
                                         (upper[i]^8 * x[i]) / 2 -
                                         x[i] / 2 + upper[i]^9 / 9 + 128 / 315)
      ...
      for (i in seq_along(x)) {
        ...
        if (x[i] >= 0 & x[i] <= 2) {
          if (upper[i] <= -1) {
            ...
            ret[i] <- f2(x = x[i], upper = x[i] + 1) + f1(x = x[i], upper = 1) +
              (upper[i] - x[i] - 1)
          }
        } else if (x[i] >= -2 & x[i] <= 0) {
          if (upper[i] <= x[i] - 1) {
            ...
            ret[i] <- f4(x = x[i], upper = upper[i]) + f5(x = x[i], upper = x[i] + 1)
          } else if (upper[i] >= 1) {
            ret[i] <- f5(x = x[i], upper = 1) + f4(x = x[i], upper = x[i] + 1) + (upper[i] - 1)
          }
        } else if (x[i] >= 2) {
          if (upper[i] <=  -1) {
            ret[i] <- 0
