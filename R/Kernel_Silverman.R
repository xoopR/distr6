        ...
        if (x[i] >= 0) {
          if(upper[i] == Inf) {
            ...
            ret[i] = (exp((2 * upper[i] - x[i])/sqrt(2)) / (16 * sqrt(2))) *
                      (cos((2 * upper[i] - x[i])/sqrt(2)) + 2 * cos(x[i] / sqrt(2)) -
                      sin((2 * upper[i] - x[i]) / sqrt(2)))
          } else if (upper[i] >= 0 & upper[i] <= x[i]) {
            ret[i] = (exp(- x[i]  / sqrt(2)) / (16 * sqrt(2))) *
                    (sin(x[i] / sqrt(2)) + 3 * cos(x[i] / sqrt(2)) +
                    (sin((2 * upper[i] - x[i]) / sqrt(2)) + (2* upper[i]  +  sqrt(2)) * sin(x[i] / sqrt(2))))
          } else if (upper[i] >= x[i]) {
          ...
          if (upper[i] == Inf) {
            ret[i] = (exp(x[i] / sqrt(2)) * (-3 * sin(x[i] / sqrt(2)) + 3 *
              ...
              ret[i] = (exp(x[i]  / sqrt(2)) / (16 * sqrt(2))) *
                        ((-2 + sqrt(2) * x[i] - sqrt(2) * upper[i]) *sin(x[i] / sqrt(2)) + 3 * cos(x[i] / sqrt(2))) -
                        sin((x[i] - 2 * upper[i]) / sqrt(2))
            } else if (upper[i] >= 0) {
              ret[i] = (exp((x[i] - 2 * upper[i]) / sqrt(2)) / (16 * sqrt(2))) *
                       (sin((x[i] - 2 * upper[i]) / sqrt(2)) - cos((x[i] - 2 * upper[i]) / sqrt(2))) +
                       (exp(- x[i]  / sqrt(2)) / (8 * sqrt(2))) * (3 * cos(x[i] / sqrt(2)) - (3 - sqrt(2) * x[i])* sin(x[i] / sqrt(2)))
            }
          }
        }
      return(ret)
