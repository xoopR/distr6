.plot_discrete <- function(fun, plotStructure, name, ...){
  ...
  if("pdf" %in% fun){
    ...
    plots$pdf = c(list(x = plotStructure$points, y = plotStructure$pdf, type = "h"),
                  args)
  }
  ...
  if("cdf" %in% fun){
    ...
    plots$cdf$plot = c(list(x = plotStructure$points, y = plotStructure$cdf, type = "n"),
                       args)
  ...
  if('quantile' %in% fun){
    ...
    plots$quantile$plot = c(list(x = plotStructure$cdf, y = plotStructure$points, type = "n"),
                            args)
  ...
  if("survival" %in% fun){
    ...
    plots$survival$plot = c(list(x = plotStructure$points, y = plotStructure$survival, type = "n"),
                            args)
  ...
  if("hazard" %in% fun){
    ...
    plots$hazard = c(list(x = plotStructure$points, y = plotStructure$hazard, type = "h"),
                     args)
  }
