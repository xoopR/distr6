...
2. A description of methodology including the choice of numeric methods and a description of any packages dependended on including how their code is written (e.g. have you seen their code, is it in R or C, are there references).
...
Note: if the user has already decorated their distribution with `FunctionImputation` then even if your decorator thinks the results are analtic, they are not. However this is taken into account by the warning message in the imputed p/d/q/r methods.

## Summary

1. Decorators are simply to implement, just define a new class inheritinf from `DistributionDecorator`
