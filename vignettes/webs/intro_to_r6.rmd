...
Every distribution in distr6 is stored as a class, we will take the Normal distribution as a class. Note the R class type is an `R6ClassGenerator` this tells us that this is a class to be constructed (as opposed to an object that can be manipulated).
```{r}
class(Normal)
```

Constructing a class in R6 is always done using the same syntax `$new(...)` but the arguments past to this function depend on the class. Just like functions in R these arguments can be named or un-named and a default may or may not be provided. In this case there is a default so the following doesn't produce an error
```{r}
Normal$new()
```
...
Normal$new(var = 2)
```
See the help documentation for the specific arguments to pass `?Normal`.

## Methods

Methods work in the exact same way as function except that again the dollar sign notation is used, `object$method(...)`. Notice that the constructor `$new()` is used for classes whereas methods are called on objects. For example let's construct a Normal distribution object and save this 
```{r}
N <- Normal$new()
...
N$setParameterValue(list(var = 5))
N$getParameterValue("var")
```
Don't worry about the particulars of these methods just yet, we will return to them in later tutorials. For now just notice using the `setParameterValue()` method update the object `N` without us having to manually save this.
...
```{r}
N$setParameterValue(list(var = 6))$getParameterValue("var")
```
See how we simply added one method to the end of the other using the same dollar sign syntax. Not all methods can be chained and don't worry if you don't feel comfortable doing this, we're just highlighting it for users more familiar with object-oriented programming.

## Cloning

The final thing we discuss is an advanced topic but is very important in R6. The best way to demonstrate cloning and copying is by example and we won't discuss any of the details. 

First we create a class called "adder" don't worry about the technicals here, in summary it's a class with one method, which adds numbers to itself. 
```{r}
...
Now comes the tricky part, watch what happens when we copy the object `a` via the function `a = b`
```{r}
a <- adder$new()
b <- a
a$x
...
See what happened? Even though we only called the method on `b`, the results were copied back to `a`. Sometimes this can be very useful but more often that not it is unneccesary. To get around this problem use the `$clone()` method:
