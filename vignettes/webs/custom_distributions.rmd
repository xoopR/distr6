...
The most basic distribution that can be constructed consists of a name and one of pdf or cdf. But most of the time we will also require a `ParameterSet`. We will demonstrate all of this by using the runnign example of a custom uniform distribution.

### self

The `self` keyword is used to tell an object that it shoud call a method on itself. For example we have used the method `getParameterValue()` on objects before but often we need the object to use this method on itself, so we use `self$getParameterValue()` this is especially important when defining p/d/q/r functions.
...
We have a pdf that accesses parameters, but currently we have no parameters to access. To add these we have to first construct a `ParameterSet` objects. In fact we have seen these objects multiple times throughout these tutorials, they are what we see when we call `parameters()` on a distribution. Every time you use the `set/getParameterSet()` methods these are actually called on the `ParameterSet`. Constructing a ParameterSet is simple, you just have to remember which arguents are required, and luckily these are all given in `?ParameterSet`
...
2. By default the type, support and distrDomain are taken to be the Reals, if this is not the case then the true ones should be supplied as `SetInterval` objects.
...
U <- Distribution$new(name = "Uniform", short_name = "unif", type = Reals$new(), support = Interval$new(1, 10), distrDomain = Reals$new(), symmetric = TRUE, pdf = pdf, cdf = cdf, parameters = ps, description = "Custom uniform distribution")
...
And that's all there is to it! These tutorials have covered everything from the basics of constructing an implemented `SDistribution` right the way through, accessing and setting parameters, analysis distributions, manipulating them with decorators and wrappers, and finally adding your own custom distribution and using decorators to analyse it. The Extension Guidelines explain how to implement your own SDistribution, Kernel, Decorator or Wrapper and the Appendices include discussions about OOP, R6, C vs. R implementation, the current API lifecycle and other design decisions. We hope you find distr6 intutiive to use but if you have any questions or want to report a bug, please don't hesitate to [raise a pull-request](https://github.com/alan-turing-institute/distr6/issues). Good luck and happy coding!
