...
class 'kernel', which in turn inherits from `Distribution`. But this is a much simpler class with reduced methods and many of these return the same result and are therefore defined in the `kernel` parent class. For every kernel the following is true
...
And for all kernels the sampling method uses inverse cdf transform sampling, so this is also defined in
...
2. distrDomain will always be `Reals$new()`
3. The support will generally either be `Reals$new()` or `Interval$new(-1,1)`
4. Kernels are all symmetric
5. The constructor always takes one argument only, `decorators` which is passed to the parent-class constructor
