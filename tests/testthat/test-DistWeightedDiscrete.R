...
test_that("constructor",{
  expect_silent(WeightedDiscrete$new(data.frame = data.frame(samples = 1, pdf = 1, cdf = 1)))
  expect_error(WeightedDiscrete$new(data.frame = data.frame(samples = 1, pdf = 2, cdf = 1)))
  expect_error(WeightedDiscrete$new(data.frame = data.frame(samples = 1, pdf = 1, cdfs = 1)))
  expect_error(WeightedDiscrete$new(data.frame = data.frame(samples = 1)))
  expect_error(WeightedDiscrete$new(data.frame = data.frame(sample = 1)))
})

gd = WeightedDiscrete$new(data.frame(samples = 1:10, pdf = rep(0.1,10)))
...
test_that("statistics",{
  ...
  expect_equal(WeightedDiscrete$new(data.frame(samples = 1:3,
                                                  cdf = c(1/5,4/5,1)))$pdf(1:3), c(1/5,3/5,1/5))
  expect_equal(WeightedDiscrete$new(data.frame(samples = 1:3,
                                                  pdf = c(1/5,3/5,1/5)))$cdf(1:3), c(1/5,4/5,1))
  set.seed(42)
  x = round(runif(1000, 0, 50))
  gd = WeightedDiscrete$new(data.frame(samples = sort(unique(x)),
