...
test_that("check continuous Truncated wrapper", {
  continuousTester2 = TruncatedDistribution$new(continuousTester, lower = 1, upper = 5)
  expect_equal(continuousTester2$support()$numeric(), 1:5)
  expect_equal(continuousTester2$pdf(0), 0)
  expect_equal(continuousTester2$pdf(6), 0)
  continuousTester3 = TruncatedDistribution$new(continuousTester)
