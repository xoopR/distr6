...
#   expect_silent(expect_equal(continuousTester$cumHazard(2, T), log(-pexp(2, log.p = T, lower.tail = F))))
# })
...
test_that("p-norms", {
  ...
  expect_message(expect_equal(continuousTester$pdfSquared2Norm(), continuousTester$pdfPNorm(2)^2))
  expect_message(expect_equal(continuousTester$pdfSquared2Norm(lower = 2, upper = 5), continuousTester$pdfPNorm(2, 2, 5)^2))
})
