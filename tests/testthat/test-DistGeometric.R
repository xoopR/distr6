...
test_that("statistics",{
    ...
    expect_equal(Geometric$new(success = T)$mean(), 2)
    ...
    expect_equal(Geometric$new(success = T)$mode(),1)
    expect_equal(round(g$entropy(exp(1)), 5), round((-0.5 * log(0.5) - 0.5 * log(0.5)) * 2,5))
    expect_equal(g$mgf(1), 0.5/(1-0.5*exp(1)))
    expect_equal(Geometric$new(success = T)$mgf(0.1), (0.5*exp(0.1))/(1-0.5*exp(0.1)))
    expect_equal(Geometric$new(success = T)$mgf(1), NaN)
    expect_equal(g$cf(1), 0.5/(1-0.5*exp(1i)))
    expect_equal(Geometric$new(success = T)$cf(0.1), (0.5*exp(0.1i))/(1-0.5*exp(0.1i)))
    expect_equal(g$pgf(3),-1)
    expect_equal(Geometric$new(success = T)$pgf(3), -3)
    ...
    expect_equal(Geometric$new(success = T)$pdf(1), dgeom(2, 0.5))
    expect_equal(Geometric$new(success = T)$cdf(1), pgeom(2, 0.5))
    expect_equal(Geometric$new(success = T)$quantile(0.324), qgeom(0.324, 0.5)+1)
    expect_silent(Geometric$new(success = T)$rand(10))
})
