...
test_that("parameterisation",{
  ...
  expect_silent(ChiSquared$new(df = 10, location = 5))
  expect_error(ChiSquared$new(df = -1))
  expect_error(ChiSquared$new(df = 1, location = -1))
  expect_equal(ChiSquared$new(df = 10)$getParameterValue("df"), 10)
  expect_equal(ChiSquared$new(df = 10, location = 5)$getParameterValue("location"), 5)
})
...
test_that("properties & traits", {
  ...
  expect_equal(ChiSquared$new(df=2, location = 3)$dmax(), Inf)
  expect_equal(ChiSquared$new(df=2, location = 3)$dmin(), .Machine$double.eps)
})

chi <- ChiSquared$new(df = 8)
chi_nc <- ChiSquared$new(df = 5, location = 2)
test_that("statistics", {
  expect_equal(chi$mean(), 8)
  expect_equal(chi_nc$mean(), 7)
  expect_equal(chi$variance(), 16)
  expect_equal(chi_nc$variance(), 18)
  expect_equal(chi$skewness(), 1)
  expect_equal(chi_nc$skewness(), 22*sqrt(2)/27)
  expect_equal(chi$kurtosis(TRUE), 1.5)
  expect_equal(chi_nc$kurtosis(TRUE), 156/81)
  expect_equal(chi$kurtosis(FALSE), 4.5)
  expect_equal(chi_nc$kurtosis(FALSE), 156/81 + 3)
  expect_equal(round(chi$entropy(), 5), 3.81661)
  expect_equal(chi_nc$entropy(), NaN)
  expect_equal(chi$mgf(0), 1)
  expect_equal(chi_nc$mgf(-.5), exp(-.5)/(2^(5/2)))
  expect_equal(chi$mgf(3), NaN)
  expect_equal(chi_nc$mgf(3), NaN)
  expect_equal(chi$cf(1/2), as.complex(-0.25))
  expect_equal(chi_nc$cf(1/2), exp(1i/(1 - 1i))/((1 - 1i)^(5/2)))
  expect_equal(chi$pgf(1/2), (1 - log(0.5)*2)^(-4))
  expect_equal(chi$pgf(3), NaN)
  expect_equal(chi_nc$pgf(1), NaN)
  expect_equal(chi$mode(), 6)
  expect_equal(chi_nc$mode(), NaN)
  expect_equal(chi$pdf(1), dchisq(1, 8))
  expect_equal(chi_nc$pdf(1), dchisq(1, 5, 2))
  expect_equal(chi$cdf(1), pchisq(1, 8))
  expect_equal(chi_nc$cdf(1), pchisq(1, 5, 2))
