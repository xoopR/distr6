...
test_that("check huberization constructor", {
  ...
  expect_equal(huberize(Binomial$new(), lower = -5, upper = 20)$getParameterValue("hub__lower"), 0)
  expect_equal(huberize(Binomial$new(), lower = -5, upper = 20)$getParameterValue("hub__upper"), 10)
...
test_that("check huberization parameters", {
  ...
  expect_equal(x$getParameterValue("hub__lower"), 1)
  expect_equal(x$getParameterValue("hub__upper"), 5)
  expect_silent(x$setParameterValue(hub__lower = 2))
  expect_silent(x$setParameterValue(hub__upper = 10))
  expect_equal(x$inf, 2)
  expect_equal(x$sup, 10)
  expect_equal(x$getParameterValue("hub__lower"), 2)
  expect_equal(x$getParameterValue("hub__upper"), 10)
  expect_error(x$setParameterValue(hub__upper = 1))
  expect_error(x$setParameterValue(hub__lower = 12))
  expect_error(x$setParameterValue(hub__lower = 4, hub__upper = 3))
  expect_true(testInterval(x$properties$support))
  x <- huberize(Exponential$new(), lower = 1, upper = 5)
  expect_silent(x$setParameterValue(hub__lower = 2, hub__upper = 10))
  expect_equal(x$inf, 2)
  expect_equal(x$sup, 10)
  expect_true(testInterval(x$properties$support))
})
